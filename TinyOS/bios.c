...
void cpu_core_halt()
{
	...
	Core* core = curr_core();
	assert(! core->int_disabled); 	//Assert interrupts are not disabled.
	...
	//Push in the fron of "halted_list" ring list this core.
	rlist_push_front(&halted_list, & core->halted_node); 

	//Waits for specified condition on first variable , mutex goes on the second ????
	while(core->halted)
