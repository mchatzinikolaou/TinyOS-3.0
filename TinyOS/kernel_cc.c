...
/*Acquire the lock */
//Test the lock. Return the value and set it to true.
/*
	If the lock was FALSE, set it to TRUE and continue your code. (goto finish, then execute rest of code.)

	If the lock was TRUE , get in the sub_code. This is equivalent to a spinlock. We will only escape after the 
	thread being executed releases this lock.
...
*/
  ...
  while(__atomic_test_and_set(lock,__ATOMIC_ACQUIRE)) { 
    ...
    /*Acquire the lock without inter-thread constraints*/

     /*
	If the lock unlocks in the main thread, we will escape here.
	Otherwise we spin.
     */
    ...
    while(__atomic_load_n(lock, __ATOMIC_RELAXED)) {
      ...
      if(spin>0) 
      	...
      	if(get_core_preemption()) 
      		//If you are preemptive, yield control.
      		yield(SCHED_MUTEX); 
      }
