...
BOOT_TEST(my_custom_thread_test,
	""
	)
{
	int flag = 0;
	...
	Tid_t t[10];
	for(int i=0;i<10;i++){
		t[i]=CreateThread(task, sizeof(flag),&flag);
		ThreadJoin(t[i],&flag);
	}
...
void connect_sockets(Fid_t sock1, Fid_t lsock, Fid_t* sock2, port_t port)
{
	...
	int connect_thread(int argl, void* args) {
		fprintf(stderr,"connect started.\n");
		ASSERT(Connect(sock1, port, 1000)==0);
		fprintf(stderr,"connect ended.\n");
		return 0;
	}
	Tid_t t1,t2;
	...
	)
{
	...
	//We can't access the streamobj for f=0 (why?)
	Socket(100);

	Fid_t f = Socket(100);
	ASSERT(Listen(f)==0);
	ASSERT(Listen(Socket(100))==-1);
	fprintf(stderr, "f has value %d\n",f );
	Close(f);
	ASSERT(Listen(Socket(100))==0);	
	return 0;
}
